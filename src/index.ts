	...
	for (let i = 0; i < originalMessage.length; i++) {
		for (let j = 0; j < alphaLatin.length; j++) {
			if (originalMessage[i] === alphaLatin[j]) {
				...
				(originalMessage.charCodeAt(i) >= 9 && originalMessage.charCodeAt(i) <= 11) ||
				(originalMessage.charCodeAt(i) > 32 && originalMessage.charCodeAt(i) < 39) ||
				(originalMessage.charCodeAt(i) > 39 && originalMessage.charCodeAt(i) <= 64) ||
				(originalMessage.charCodeAt(i) >= 91 && originalMessage.charCodeAt(i) < 96) ||
				(originalMessage.charCodeAt(i) >= 123 && originalMessage.charCodeAt(i) <= 1300)
			) {
	...
	for (let i = 0; i < CyrillicMessage.length; i++) {
		for (let j = 0; j < alphaRus.length; j++) {
			if (CyrillicMessage[i] === alphaRus[j]) {
				...
				(CyrillicMessage.charCodeAt(i) >= 9 && CyrillicMessage.charCodeAt(i) <= 11) ||
				(CyrillicMessage.charCodeAt(i) > 32 && CyrillicMessage.charCodeAt(i) < 1000) ||
